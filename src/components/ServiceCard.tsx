...
const ServiceCard = ({ AnimationData, heading,text,color,techName }: Props) => {
  ...
  return (
    ...
    {text}
        ...
        <div className="grid grid-flow-row grid-cols-2 lg:grid-cols-3 max-w-sm gap-3">
            {techName.map((item)=>(
                 <TechStackButton techName={item} />
            ))}
        </div>
      </div>
        <Lottie className="max-w-[300px] max-h-[300px] self-center"  animationData={AnimationData}/>
    </div>
  );
};
